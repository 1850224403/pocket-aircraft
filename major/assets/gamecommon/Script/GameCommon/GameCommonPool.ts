export class GameCommonPool {
    ...
    //存放可用的实例
    private static currentPool: Map<cc.Prefab, Array<cc.Node>> = new Map();
    ...
    public static addPrefabPool(prefab: cc.Prefab): void {
        ...
        if (this.currentPool.has(prefab)) {
        ...
        this.currentPool.set(prefab, currentList);
    }
    ...
    public static requestInstantWithArgs(prefab: cc.Prefab, position: cc.Vec2, rotation: number, parent: cc.Node): cc.Node {
        ...
        var currentList = this.currentPool.get(prefab);
     ...
     * @param cleanup 默认true, 传入true时，这个节点上所有绑定的事件、action 都会被删除。
     */
    public static returnInstant(instant: cc.Node, cleanup: boolean = true): void {
        ...
        instant.removeFromParent(cleanup);
    ...
    public static reset(): void {
        ...
        for (var instantList of Array.from(this.currentPool.values())) {
